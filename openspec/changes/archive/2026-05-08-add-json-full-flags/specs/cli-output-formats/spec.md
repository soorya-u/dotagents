...
When `--content` is passed to `commands ls` or `skills ls`, the command SHALL include the full markdown body content of each item in the output. In TTY mode, each item is rendered as a `cliclack::note` block: the note header shows the item name (styled green+bold) followed by ` — ` and the description; the note body contains the raw markdown content. No separate `info!` line is printed for items that have body content. In non-TTY mode, body lines are printed indented below the name-description row. Without `--content`, only the name and frontmatter fields SHALL be shown.
...
In TTY mode, the item name SHALL be rendered in cyan+bold using `console::style`. The name and description SHALL be separated by ` — `. Column width SHALL match the actual longest name in the result set with no artificial minimum padding.
...
JSON output from `--json` SHALL be valid JSON that can be piped to tools like `jq`. Each object SHALL include at least a stable identifier field (e.g., `name`). The `content` field SHALL be included only when `--content` is also passed. The output SHALL NOT include any non-JSON text on stdout.
