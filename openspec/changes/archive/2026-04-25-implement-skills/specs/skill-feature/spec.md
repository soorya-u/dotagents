...
The system SHALL warn (via log) when a skill directory name does not match the `name` frontmatter in its `SKILL.md`, but SHALL still load the skill. The system SHALL also warn and skip any subdirectory that contains no `SKILL.md`.

#### Scenario: Directory name matches frontmatter name

- **WHEN** the skill directory is `pdf-processing/` and frontmatter `name` is `pdf-processing`
- **THEN** the system SHALL load the skill without any warning

#### Scenario: Directory name does not match frontmatter name

- **WHEN** the skill directory is `pdf/` but frontmatter `name` is `pdf-processing`
- **THEN** the system SHALL log a warning and still load the skill successfully

#### Scenario: Subdirectory missing SKILL.md

- **WHEN** a subdirectory exists in `skills/` but contains no `SKILL.md`
- **THEN** the system SHALL log a warning and skip that directory

### Requirement: Skills directory loading

The system SHALL load skills by scanning subdirectories of `.dotagents/skills/`. Each subdirectory must contain a `SKILL.md` file whose content is parsed as a `SkillFeature`. Non-directory entries are skipped silently.

#### Scenario: Multiple skills in directory

- **WHEN** the `skills/` directory contains multiple subdirectories each with a `SKILL.md`
- **THEN** the system SHALL load each as a separate `SkillFeature` instance
...
- **THEN** a sample skill SHALL be created at `skills/hello-skill/SKILL.md` inside the `.dotagents/` directory
