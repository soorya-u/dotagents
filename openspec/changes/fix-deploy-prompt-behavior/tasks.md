...
- [ ] 4.2 Add e2e test in `tests/e2e/commands.test.ts`: `commands new NAME --ci --offline` with local provider — assert deployed output file is created (TC-CMD-NEW-CI-DEPLOY)
...
- [ ] 4.5 Add e2e test in `tests/e2e/skills.test.ts`: `skills new NAME --ci --offline` with local provider — assert deployed output file is created
