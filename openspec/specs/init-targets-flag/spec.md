...
Specifies the `--targets` flag for `dotagents init`, which allows users to select provider targets from the command line without the interactive wizard prompt.
