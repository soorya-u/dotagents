...
Specifies the cleanup behaviour that runs when `dotagents skills rm` or `dotagents commands rm` removes a source item. Cleanup removes all deployed files, cache entries, and `.gitignore` fence entries for the item across every provider.
