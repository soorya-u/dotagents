...
After listing affected paths, the command SHALL print `N files would be affected` where N is the count of unique target paths (after dedup), not the count of providers. When no files would be affected, the count SHALL be 0.
