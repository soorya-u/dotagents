...
The system SHALL provide skill template and target path configurations in `public/v1/templates/` for all supported providers, with targets following the Agent Skills specification directory structure.
...
### Requirement: Claude Code extension fields
The system SHALL support Claude Code's skill frontmatter extensions: `disable-model-invocation` (boolean, optional), `user-invocable` (boolean, optional), and `paths` (list of strings, optional). These fields SHALL be parsed from source skills and rendered only in provider templates that support them.

#### Scenario: Parse disable-model-invocation
- **WHEN** a skill file has `disable-model-invocation: true` in frontmatter
- **THEN** the system SHALL parse it as `Some(true)` on `SkillMetadata.disable_model_invocation`

#### Scenario: Parse user-invocable
- **WHEN** a skill file has `user-invocable: false` in frontmatter
- **THEN** the system SHALL parse it as `Some(false)` on `SkillMetadata.user_invocable`
...
- **THEN** the rendered output SHALL contain `disable-model-invocation: true` in the frontmatter

#### Scenario: Deploy user-invocable to factory-droid
- **WHEN** a skill with `user-invocable: false` is deployed to the factory-droid provider
- **THEN** the rendered output SHALL contain `user-invocable: false` in the frontmatter

#### Scenario: Deploy paths to claude
- **WHEN** a skill with `paths: ["src/**/*.rs"]` is deployed to the claude provider
- **THEN** the rendered output SHALL contain a `paths:` YAML list with `- src/**/*.rs`

#### Scenario: Omit extension fields for unsupported providers
- **WHEN** a skill with `disable-model-invocation: true` is deployed to the codex provider
- **THEN** the rendered output SHALL NOT contain `disable-model-invocation`

### Requirement: Metadata rendering in all provider templates
The system SHALL render the `metadata` field in all provider skill templates as a YAML key-value map when present.

#### Scenario: Deploy metadata to any provider
- **WHEN** a skill with `metadata: {author: test, version: "1.0"}` is deployed to any provider
- **THEN** the rendered output SHALL contain:
  ```yaml
  metadata:
    author: test
    version: "1.0"
...
#### Scenario: Omit metadata when absent
- **WHEN** a skill with no `metadata` field is deployed
- **THEN** the rendered output SHALL NOT contain a `metadata:` key
