...
**Migration**: No action needed; truncation and `--content` behaviour are unchanged on the replacement subcommands.

### Requirement: Verbose flag shows full descriptions
**Reason**: Removed with the top-level `ls` command. Additionally, the implicit tie-in between the global `-v`/`--verbose` flag and full-description display is removed entirely as incorrect behaviour. Full descriptions are now shown only via the explicit `--content` flag on `commands ls` and `skills ls`.
**Migration**: Replace `dotagents ls --verbose` with `dotagents commands ls --content` or `dotagents skills ls --content` as appropriate.
