...
Defines the deploy-mode behaviour for features in `dotagents deploy`. Features can be deployed in `link` mode (create filesystem symlinks from `.dotagents/` sources to provider target paths, no content rendering) or `template` mode (inject variables and render through `.hbs` templates, the existing behaviour). Mode is configurable per feature and per item, and feature types are classified as symlinkable (Type 1: skills, agent-ignore, agent configs) or non-symlinkable (Type 2: commands, instructions, MCP), which determines how each mode is interpreted.
