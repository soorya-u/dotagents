...
E2e test for the interactive TUI provider list select widget.
...
- **WHEN** `providers --offline` is run interactively in a TTY with a seeded registry cache
- **THEN** the select widget renders with "Providers" title, provider entries are visible, and Enter selects a provider showing an outro with the provider name and URL
