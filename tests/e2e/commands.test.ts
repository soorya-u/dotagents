...
test.describe("commands --cwd", () => {
	...
	test("relative --cwd resolved against current directory", async () => {
		...
		try {
			...
			run(["init", sub, "--template", "starter", "--features", "commands,instructions,mcp,skills"]);
...
test.describe("commands new TUI – T07 deploy on Yes", () => {
	const d = makeTmpDir();
	run(["init", "--template", "with-custom-provider", "--features", "commands,instructions,mcp,skills"], d);
