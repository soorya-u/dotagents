...
import { BIN, cleanup, makeTmpDir, run, shellProgram } from "./helpers.js";
...
test.describe("add command CLI", () => {
  ...
  test("all flags populate frontmatter fields", async () => {
    ...
    try {
      ...
      const { exitCode } = run(
        ["add", "command", "greet", "--description", "Say hello", "--category", "Greetings", "--tags", "greeting,hello"],
        d,
      );
      expect(exitCode).toBe(0);
      const content = readFileSync(join(d, ".dotagents-debug/commands/greet.md"), "utf8");
  ...
  test("--force overwrites existing command file", async () => {
    ...
    try {
      ...
      const { exitCode } = run(["add", "command", "greet", "--description", "second", "--force"], d);
      expect(exitCode).toBe(0);
      const content = readFileSync(join(d, ".dotagents-debug/commands/greet.md"), "utf8");
  ...
  test("duplicate without --force exits non-zero", async () => {
    ...
    try {
      run(["init", "--template", "starter"], d);
      run(["add", "command", "greet", "--description", "first"], d);
      const { exitCode, stderr } = run(["add", "command", "greet", "--description", "second"], d);
...
test.describe("add skill CLI", () => {
  ...
  test("all flags populate frontmatter fields", async () => {
    ...
    try {
      run(["init", "--template", "starter"], d);
      const { exitCode } = run(
        ["add", "skill", "my-skill", "--description", "Greet users", "--license", "MIT", "--compatibility", "Requires node"],
        d,
      );
      expect(exitCode).toBe(0);
      const content = readFileSync(join(d, ".dotagents-debug/skills/my-skill/SKILL.md"), "utf8");
  ...
  test("--force overwrites existing skill", async () => {
    ...
    try {
      ...
      const { exitCode } = run(["add", "skill", "my-skill", "--description", "second", "--force"], d);
      expect(exitCode).toBe(0);
      const content = readFileSync(join(d, ".dotagents-debug/skills/my-skill/SKILL.md"), "utf8");
      expect(content).toContain('"second"');
    } finally {
...
test.describe("add command TUI – T06 interactive prompts", () => {
  ...
  test("prompts for description, category, tags then deploy", async ({ terminal }) => {
    try {
      await expect(terminal.getByText("dotagents add command")).toBeVisible();
      await expect(terminal.getByText("Description")).toBeVisible();
      await expect(terminal.getByText("What does this command do?")).toBeVisible();
      ...
      expect(existsSync(join(d, ".dotagents-debug/commands/greet.md"))).toBe(true);
      const content = readFileSync(join(d, ".dotagents-debug/commands/greet.md"), "utf8");
...
test.describe("add command TUI – T07 deploy on Yes (skipped)", () => {
  ...
  test.skip("answering Yes to deploy prompt runs deploy (deploy prompt is nested inside add)", async ({ terminal }) => {
    try {
      await expect(terminal.getByText("Description")).toBeVisible();
...
test.describe("add skill TUI – T08 interactive prompts", () => {
  ...
  test("prompts for description, license, compatibility", async ({ terminal }) => {
    try {
      ...
      expect(existsSync(join(d, ".dotagents-debug/skills/my-skill/SKILL.md"))).toBe(true);
    } finally {
      cleanup(d);
    }
  });
