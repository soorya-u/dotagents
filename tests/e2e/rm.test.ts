...
test.describe("rm command CLI", () => {
  ...
  test("--force deletes command file", async () => {
    ...
    try {
      ...
      expect(existsSync(join(d, ".dotagents-debug/commands/hello.md"))).toBe(true);
      const { exitCode } = run(["rm", "command", "hello", "--force"], d);
      expect(exitCode).toBe(0);
      expect(existsSync(join(d, ".dotagents-debug/commands/hello.md"))).toBe(false);
    } finally {
      cleanup(d);
    }
  });
...
test.describe("rm skill CLI", () => {
  ...
  test("--force deletes skill directory", async () => {
    ...
    try {
      run(["init", "--template", "starter"], d);
      expect(existsSync(join(d, ".dotagents-debug/skills/hello-skill"))).toBe(true);
      const { exitCode } = run(["rm", "skill", "hello-skill", "--force"], d);
      expect(exitCode).toBe(0);
      expect(existsSync(join(d, ".dotagents-debug/skills/hello-skill"))).toBe(false);
    } finally {
      cleanup(d);
    }
  });
...
test.describe("rm skill TUI – T12 confirm Yes", () => {
  ...
  test("confirm Yes removes the skill", async ({ terminal }) => {
    try {
      await expect(terminal.getByText("Remove skill 'hello-skill'?")).toBeVisible();
