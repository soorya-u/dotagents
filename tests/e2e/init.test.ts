...
test.describe("init CLI – file tree", () => {
  ...
  test("with-custom-provider adds template files", async () => {
    const d = makeTmpDir();
    try {
      const { exitCode } = run(["init", "--template", "with-custom-provider"], d);
      expect(exitCode).toBe(0);
      const root = join(d, ".dotagents-debug");
      expect(existsSync(join(root, "templates/mycode/command.hbs"))).toBe(true);
      expect(existsSync(join(root, "templates/mycode/instructions.hbs"))).toBe(true);
  ...
  test("--no-instruction omits INSTRUCTIONS.md", async () => {
    ...
    try {
      ...
      expect(existsSync(join(d, ".dotagents-debug/INSTRUCTIONS.md"))).toBe(false);
    } finally {
      cleanup(d);
    }
  });
...
test.describe("init CLI – config content", () => {
  test("config.toml declares expected features", async () => {
    ...
    try {
      run(["init", "--template", "starter"], d);
      const config = readFileSync(join(d, ".dotagents-debug/config.toml"), "utf8");
  ...
  test("hello.md has YAML frontmatter with name and description", async () => {
    ...
    try {
      run(["init", "--template", "starter"], d);
      const cmd = readFileSync(join(d, ".dotagents-debug/commands/hello.md"), "utf8");
      expect(cmd).toMatch(/^---/);
...
test.describe("init TUI – T01 wizard happy path", () => {
  ...
  test("wizard shows all prompts and completes successfully", async ({ terminal }) => {
    try {
      ...
      terminal.keyPress("Enter");
      await expect(terminal.getByText("Which providers would you like to target?")).toBeVisible();
...
test.describe("init TUI – T03 WithCustomProvider template", () => {
  ...
  test("selecting WithCustomProvider template creates mycode templates", async ({ terminal }) => {
    try {
      await expect(terminal.getByText("Which features do you want to enable?")).toBeVisible();
      terminal.keyPress("Enter");
      await expect(terminal.getByText("Which starting template?")).toBeVisible();
      ...
      expect(existsSync(join(d, ".dotagents-debug/templates/mycode/command.hbs"))).toBe(true);
    } finally {
      cleanup(d);
    }
  });
...
test.describe("init TUI – T05 overwrite cancel (skipped)", () => {
  ...
  test.skip("cancel overwrite exits without changes (release binary only)", async ({ terminal }) => {
    try {
